...
export default function MobileAppDock() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("kamalia-guide-opened", opened);
    window.addEventListener("kamalia-guide-closed", closed);
    return () => { window.removeEventListener("kamalia-guide-opened", opened); window.removeEventListener("kamalia-guide-closed", closed); };
  }, []);
  ...
  return <>
    <nav aria-label={t("التنقل الرئيسي", "Primary navigation")} className="app-dock fixed inset-x-0 bottom-0 z-[700] px-2 pb-[max(.5rem,env(safe-area-inset-bottom,0px))] lg:hidden">
      <div className="mx-auto grid h-[4.75rem] w-full max-w-2xl grid-cols-5 items-stretch rounded-[1.4rem] border border-navy-100/90 bg-white/95 p-1.5 shadow-float backdrop-blur-xl">
        ...
        <button ref={guideButton} type="button" onClick={() => window.dispatchEvent(new CustomEvent("kamalia-guide-open"))} aria-haspopup="dialog" className="flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-2xl bg-brand-600 px-1 text-white shadow-soft transition active:scale-[.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-400 focus-visible:ring-offset-2"><Icon name="navigation" className="h-5 w-5" /><span className="text-[10px] font-bold leading-tight sm:text-xs">{t("الدليل", "Guide")}</span></button>
