...
function ArrowBtn({ dark = true }: { dark?: boolean }) {
  return (
    <span className={`flex h-9 w-9 items-center justify-center self-end rounded-full border md:h-12 md:w-12 ${dark ? "border-navy-900 text-navy-900" : "border-white text-white"}`}>
    ...
    >
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-1.5 md:grid-cols-2 md:gap-2">
        ...
        <div className="flex flex-col gap-1.5 md:gap-2">
          ...
          >
            ...
            <div className="flex flex-wrap items-end justify-between gap-3">
              <p className="max-w-xs text-xs font-semibold text-navy-600 md:text-sm">
                {t("اختر الخدمة، الوقت، وسنتواصل معك لتأكيد التفاصيل.", "Pick a service and time; we'll confirm the details with you.")}
              </p>
              <Link href="/booking" className="btn-primary px-6 py-3">
                {t("ابدأ الحجز", "Start booking")} <Icon name="arrow" className="h-4 w-4 rtl:rotate-180" />
              </Link>
            </div>
          </div>
          ...
          >
            <div>
              <p className="mb-2 text-xs font-semibold text-cyan-300 md:text-sm">{t("استشارة طبية", "Medical consultation")}</p>
              <h3 className="text-xl font-bold leading-7 md:text-3xl">
                {t("واتساب أو مكالمة قريبة منك", "WhatsApp or a call, close to you")}
              </h3>
            </div>
            <a
        ...
        <div className="relative min-h-[360px] overflow-hidden rounded-xl md:min-h-0 md:rounded-2xl" style={getAnimStyle(3)}>
          ...
          <div className="absolute inset-x-3 bottom-3 z-10 flex gap-1.5 md:inset-x-5 md:bottom-5 md:gap-2">
            <Link href="/booking" className="flex h-36 flex-1 flex-col justify-between rounded-xl bg-white p-3 transition-transform hover:scale-[1.02] md:h-52 md:rounded-2xl md:p-5">
            ...
            <Link href="/contact" className="flex h-36 flex-1 flex-col justify-between rounded-xl bg-white/20 p-3 backdrop-blur-xl transition-transform hover:scale-[1.02] md:h-52 md:rounded-2xl md:p-5">
