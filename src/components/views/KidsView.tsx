    ...
    <KidsGameShell back={false}>
        ...
        >
          <div className="flex flex-col items-center gap-5 text-center sm:flex-row sm:text-start rtl:sm:flex-row-reverse">
            ...
            <div className="flex-1">
              ...
              <h2 className="mt-1 text-h2 font-extrabold text-navy-900">{L(FEATURED_GAME.title)}</h2>
      ...
      <section className="mx-auto mt-14 max-w-2xl">
        <GlassCard className="text-center">
          ...
          <h2 className="text-h3 font-extrabold text-navy-900">{t("راحة بال للوالدين", "Peace of mind for parents")}</h2>
      ...
      <section className="mx-auto mt-12 max-w-2xl">
        <div className="grid items-center gap-6 sm:grid-cols-2">
          <div>
            <h2 className="text-h3 font-extrabold text-navy-900">{t("بطاقة تشجيعية لكل بطل", "An encouragement card for every champion")}</h2>
          ...
          <div className="rounded-[1.75rem] border-4 border-white bg-gradient-to-br from-brand-50 to-mint-50 p-6 text-center shadow-float">
            ...
            <p className="mt-2 text-lg font-extrabold text-navy-900">{t("اسم الطفل", "Child's name")}</p>
            <p className="text-sm font-semibold text-mint-500">{t("اسم اللعبة", "Game name")}</p>
            <div className="mx-auto mt-3 grid h-16 w-16 grid-cols-3 grid-rows-3 gap-0.5 rounded-lg bg-white p-1.5 shadow-soft" aria-hidden>
              {Array.from({ length: 9 }).map((_, i) => (
                <span key={i} className={`rounded-[2px] ${[0, 2, 4, 5, 6, 8].includes(i) ? "bg-navy-900" : "bg-transparent"}`} />
              ))}
            </div>
            <p className="mt-3 text-xs font-bold text-brand-700">Al Kamalia Medical Center</p>
          </div>
        </div>
      </section>
    </KidsGameShell>
...
function GameCard({
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-4 text-lg font-extrabold text-navy-900">{L(game.title)}</h3>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        {game.skills.map((s, i) => (
          <span key={i} className="rounded-full border border-navy-100 bg-white px-2.5 py-0.5 text-xs font-semibold text-navy-500">
            {L(s)}
          </span>
        ))}
      </div>
