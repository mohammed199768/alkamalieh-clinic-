...
export default function IqCertificateCard({
  ...
  const cardLine = lang === "ar" ? result.tier.cardLine.ar : result.tier.cardLine.en;
  ...
  const download = () => {
    ...
    ctx.fillText("AL KAMALIA MEDICAL CENTER", W / 2, 150);

    ctx.fillStyle = "#10203A";
    ctx.font = "bold 44px system-ui, sans-serif";
    ctx.fillText(lang === "ar" ? "بطاقة اعرف نفسك" : "Know Yourself Card", W / 2, 220);
    ...
    ctx.fillText(lang === "ar" ? "مؤشر المنطق البصري" : "Visual Logic Index", W / 2, 595);
    ...
    const finish = () => {
      ...
      ctx.fillText(
        lang === "ar"
          ? "من مركز الكمالية الطبي — مساحة تفاعلية للتثقيف والاكتشاف الذاتي."
          : "From Al Kamalia Medical Center — an interactive space for learning and self-discovery.",
        W / 2,
        1085
      );
    ...
    <div className="mx-auto max-w-sm">
      <div className="relative overflow-hidden rounded-[1.75rem] border border-[#2563EB]/30 bg-white p-6 text-center shadow-[0_24px_60px_-32px_rgba(16,32,58,0.4)]">
        ...
        <div className="relative rounded-[1.4rem] border border-[#E6DED2] p-5">
          ...
          <h3 className="mt-3 text-2xl font-extrabold text-[#10203A]">{displayName}</h3>
          <p className="mt-1 text-sm font-semibold text-[#2563EB]">{tierLabel}</p>

          <div className="my-4 flex items-center justify-center gap-2">
            <span className="text-4xl font-extrabold text-[#10203A]">{result.percent}%</span>
            <span className="text-xs font-medium text-[#5F6B7A]">
              {t("مؤشر المنطق البصري", "Visual Logic Index")}
            </span>
          </div>

          <p className="mb-3 text-sm font-semibold text-[#0F766E]">{cardLine}</p>

          <div ref={qrRef} className="mx-auto w-fit rounded-2xl border border-[#E6DED2] bg-white p-2.5">
